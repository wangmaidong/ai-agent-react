...
export const LibraryLoader = (() => {
  ...
  const registry = (libConfig: iLibraryLoadConfig) => {
    libConfigs.push(libConfig);
  };
  ...
  const load = async (name: string) => {
    if (!Win[GLOBAL_NAME.RESOURCE]) {
      Win[GLOBAL_NAME.RESOURCE] = {};
    }
    if (!Win[GLOBAL_NAME.RESOURCE][name]) {
      Win[GLOBAL_NAME.RESOURCE][name] = (async () => {
        ...
        if (!!resourceConfig.dependencies) {
          await Promise.all(resourceConfig.dependencies.map((i) => load(i)));
        }
